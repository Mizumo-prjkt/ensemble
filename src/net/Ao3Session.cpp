...
void Ao3Session::setCookieFromWebEngine(const QNetworkCookie &cookie)
{
    if (cookie.name() == "_otwarchive_session") {
        QList<QNetworkCookie> cookies = m_cookieJar->cookiesForUrl(QUrl(QStringLiteral("https://archiveofourown.org")));
        cookies.append(cookie);
        m_cookieJar->setCookiesFromUrl(cookies, QUrl(QStringLiteral("https://archiveofourown.org")));
...
void Ao3Session::setCookieFromString(const QString &rawCookieHeader)
{
    ...
    m_cookieJar->setCookiesFromUrl(cookies, QUrl(QStringLiteral("https://archiveofourown.org")));
    m_cookieJar->setCookiesFromUrl(cookies, QUrl(QStringLiteral("https://archiveofourown.org/users/login")));
}

void Ao3Session::verifyAndSetCookie(const QString &rawInput, const QString &providedUser)
{
    ...
    QNetworkReply *reply = authenticatedGet(QUrl(QStringLiteral("https://archiveofourown.org/")));
    connect(reply, &QNetworkReply::finished, this, [this, reply]() {
        if (reply->error() == QNetworkReply::NoError) {
            ...
            if (htmlStr.contains(QStringLiteral("Lost Cookie Home"))) {
                ...
                emit loginFailed(QStringLiteral("AO3 rejected session cookie (Lost Cookie Home). Please log in to AO3 in your browser and copy your fresh _otwarchive_session value."));
            } else {
                const QString user = Ao3Parser::parseUsername(html);
                if (!user.isEmpty()) {
                    m_username = user;
                    qDebug() << "[Ao3Session] Login verified successfully for user:" << m_username;
                    emit loginSucceeded(m_username);
                } else {
                    emit loginFailed(QStringLiteral("Connected to AO3, but could not detect logged-in username. Please check cookie validity."));
                }
            }
        } else {
            emit loginFailed(QStringLiteral("Failed to connect to AO3: ") + reply->errorString());
        }
        reply->deleteLater();
    });
}
...
QNetworkReply *Ao3Session::authenticatedGet(const QUrl &url)
{
    ...
    request.setRawHeader("User-Agent", "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36 Ensemble/1.0.0");
...
{
    m_cookieJar->setCookiesFromUrl(QList<QNetworkCookie>(), QUrl(QStringLiteral("https://archiveofourown.org")));
    m_username.clear();
}
